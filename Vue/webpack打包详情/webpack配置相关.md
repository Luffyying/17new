...
1. 再看项目，此时已经生成了一个package.json的文件，它就像一个项目的生产说明书
...
1. entry:项目的入口文件，output:打包后的文件存储的位置
	...
	name:entry.key,hash:哈希值,publicPath:指定了一个在浏览器中被引用的URL地址，
	对于script和
...
filename:生成的文件名，不会被template的名字代替
...
功能：创建一个在编译时可以配置的全局变量(使用场景如：根据设置的变量决定是否打包压缩及启动dev server 还是prod server)
